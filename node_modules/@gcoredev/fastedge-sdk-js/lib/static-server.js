...
function normalizePathsOrRegex(inputsArray = []) {
  return inputsArray.map((strInput) => {
...
var getIfNoneMatchHeader = (request) => (request.headers.get("If-None-Match") ?? "").split(",").map((x) => x.trim()).filter((x) => Boolean(x));
...
var getStaticServer = (serverConfig, assetCache) => {
  ...
  const findAcceptEncodings = (request) => {
    ...
    const found = (request.headers.get("accept-encoding") ?? "").split(",").map((x) => {
  ...
  const testExtendedCache = (pathname) => _extendedCache.some((x) => {
  ...
  const serveRequest = async (request) => {
    ...
    if (requestAcceptsTextHtml(request)) {
      ...
      if (spaEntrypoint != null) {
        const asset2 = _assetCache.getAsset(spaEntrypoint);
        if (asset2 != null) {
      ...
      if (notFoundPage != null) {
        const asset2 = _assetCache.getAsset(notFoundPage);
        if (asset2 != null) {
          return serveAsset(request, asset2, {
            status: 404,
            cache: "never"
